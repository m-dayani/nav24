...
namespace NAV24 {
    ...
    public:
        ...
        [[nodiscard]] virtual std::vector<OB::ObsPtr> undistort(const std::vector<OB::ObsPtr>& vpObs) const;
        virtual OB::ObsPtr distort(const OB::ObsPtr& pObs);

        [[nodiscard]] virtual WO::WoPtr unproject(const OB::ObsPtr& pt2d) const;
        [[nodiscard]] virtual OB::ObsPtr project(const WO::WoPtr& pt3d) const;
        [[nodiscard]] virtual Eigen::Vector2d project(const Eigen::Vector3d& pt3d) const;
        [[nodiscard]] virtual Eigen::Matrix<double, 2, 3> projectJac(const Eigen::Vector3d& pt3d) const;

        [[nodiscard]] cv::Mat getK_cv() const { return mpCamModel->getK_cv(); }
        Eigen::Matrix3f getK_ei() { return mpCamModel->getK_ei(); }
        cv::Mat getD_cv() { return mpCamModel->getDist(); }
        [[nodiscard]] std::vector<float> getIntrinsicsVector() const { return mpCamModel->getIntrinsicsVector(); }
