...
#undef GET_VALUE
    ...
    /*FtDtOrbSlam::FtDtOrbSlam(int _nfeatures, float _scaleFactor, int _nlevels, int _iniThFAST, int _minThFAST) :
            FtDt(_nfeatures), scaleFactor(_scaleFactor), nLevels(_nlevels),
    ...
    int FtDtOrbSlam::detect(FramePtr &pFrame) {

        auto pImgFrame = static_pointer_cast<FrameImgMono>(pFrame);
        cv::Mat image = pImgFrame->getImage()->mImage;
        vector<int> vLappingArea = {0, 1000};

        //cout << "[ORBextractor]: Max Features: " << mnFeatures << endl;
        if(image.empty())
            return -1;
        ...
        auto _keypoints = vector<cv::KeyPoint>(nkeypoints);
...
//        cout << "[ORBextractor]: extracted " << _keypoints.size() << " KeyPoints" << endl;
    ...
    void FtDtOrbSlam::setup(const MsgPtr &configMsg) {
